...
rawPredictFun <- function(x,config_list,coef_vec) {
  ...
  for(i in 1:nclx) {
    ...
    if(is.na(x[1,i])) {
      ...
      for(j in 1:nrow(woedf)) {
        if(x[1,i] <= woedf[j,1]) {
          woe_vec[i+1] <- woedf[j,2]
          flag <- TRUE
          break
        }
      }
    ...
    if(!flag) stop(paste("there is a value of the ",i,"th variable not in bins of 'config_list'",sep=""))
  }
...
#'
#' @description
#' \code{rawPredictFun_df} is a wrapper and parallelization of \code{\link{rawPredictFun}}, so you can predict the entire dataset at one time. Different from \code{\link{LRpredict}}, this function can predict on raw dataset directly which have not been woe-encoded or binned.
#'
#' @details
#' parameter \code{config_list} can be generated by function \code{\link{dfBinningFun}}.
#'
...
#' @title Auxiliary Function: Encode An Observation as WOE
#'
#' @description
#' Auxiliary function: \code{woeEncodeFun} will encode an observation as woe values, then return a numeric vector.
#'
#' @details
#' parameter \code{config_list} can be generated by function \code{\link{dfBinningFun}}.
#'
#' @param x A dataframe with only one row, which is the observation to be encoded.
#' @param config_list A list of woe configuration, whose every component is a dataframe corresponding to a x variable.
#'
#' @return A numeric vector
#' @family dataset binning and woe-encoding functions
#' @export
woeEncodeFun <- function(x,config_list) {
  nclx <- ncol(x)
  len_conf <- length(config_list)
  if(nclx != len_conf) stop("the length of 'config_list' is not equal to the ncol of 'x'")
  woe_vec <- numeric(nclx)
  for(i in 1:nclx) {
    flag <- FALSE
    woedf <- config_list[[i]]
    if(is.na(x[1,i])) {
    ...
    if(!flag) stop(paste("there is a value '",x[1,i],"' of the ",i,"th variable not in bins of 'config_list'",sep=""))
  }
  return(woe_vec)
}


#' @title Encode the Entire Dataset as WOE
#'
#' @description
#' \code{woeEncodeFun_df} is a wrapper and parallelization of \code{\link{woeEncodeFun}}, so you can use it to encode the entire dataset at one time.
#'
#' @details
#' parameter \code{config_list} can be generated by function \code{\link{dfBinningFun}}.
#'
...
woeEncodeFun_df <- function(x_sample,config_list) {
  ...
  clnum <- detectCores()
  cl <- makeCluster(getOption("cl.cores",clnum))
  x_sample_list_encoding <- parLapply(cl,x_sample_list,woeEncodeFun,config_list=config_list)
  stopCluster(cl)
  x_sample_encoding <- ldply(x_sample_list_encoding,.parallel=TRUE)
  x_sample_encoding <- x_sample_encoding[-1]
  names(x_sample_encoding) <- names(x_sample)
  return(x_sample_encoding)
}


#' @title Auxiliary Function: Execute Binning on An Observation
#'
#' @description
#' Auxiliary function: similar to function \code{\link{woeEncodeFun}}, base on \code{config_list} to execute binning on an observation, but will not proceed woe-encode.
#'
#' @details
#' parameter \code{config_list} can be generated by function \code{\link{dfBinningFun}}.
#'
#' @param x A dataframe with only one row, which is the observation to be binned.
#' @param config_list A list of woe configuration, whose every component is a dataframe corresponding to a x variable.
#'
#' @return A character vector
#' @family dataset binning and woe-encoding functions
#' @export
executeBinFun <- function(x,config_list) {
  nclx <- ncol(x)
  len_conf <- length(config_list)
  if(nclx != len_conf) stop("the length of 'config_list' is not equal to the ncol of 'x'")
  bin_vec <- character(nclx)
  for(i in 1:nclx) {
    flag <- FALSE
    woedf <- config_list[[i]]
    if(is.na(x[1,i])) {
      ...
      for(j in 1:nrow(woedf)) {
        if(x[1,i] <= woedf[j,1]) {
          if(j==1) {
            bin_vec[i] <- paste("(low,",woedf[j,1],"]",sep="")
          } else if(woedf[j,1]==Inf) {
            bin_vec[i] <- paste("(",woedf[j-1,1],",high)",sep="")
          } else {
            bin_vec[i] <- paste("(",woedf[j-1,1],",",woedf[j,1],"]",sep="")
          }
          flag <- TRUE
          break
        }
...
#' @title Execute Binning on Entire Dataset
#'
#' @description
#' \code{executeBinFun_df} is a wrapper and parallelization of \code{\link{executeBinFun}}, so you can use it to bin the entire dataset at one time.
#'
#' @details
#' parameter \code{config_list} can be generated by function \code{\link{dfBinningFun}}.
#'
...
executeBinFun_df <- function(x_sample,config_list) {
  ...
  clnum <- detectCores()
  cl <- makeCluster(getOption("cl.cores",clnum))
  x_sample_list_binning <- parLapply(cl,x_sample_list,executeBinFun,config_list=config_list)
  stopCluster(cl)
  x_sample_binning <- ldply(x_sample_list_binning,.parallel=TRUE)
  x_sample_binning <- x_sample_binning[-1]
  names(x_sample_binning) <- names(x_sample)
  return(x_sample_binning)
}
